...
// POST /api/academy/courses — SUPER_ADMIN only
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, categoryId, difficulty, estimatedMinutes, enforceOrder } = body;

    if (!title || typeof title !== 'string' || title.trim().length === 0) {
      return ApiErrors.validation('title is required');
    }

    const maxPos = await prisma.academyCourse.aggregate({ _max: { position: true } });
    ...
    const course = await prisma.academyCourse.create({
      data: {
        ...
        enforceOrder: enforceOrder ?? true,
